...
export function JobCard({ job }: { job: MarketplaceJob }) {
  ...
  return (
    <Link className="block rounded-[28px] bg-[#fbfff5] p-4 shadow-[0_14px_34px_rgba(17,24,15,0.09)]" href={href}>
      ...
      <p className="mt-2 line-clamp-2 text-sm font-semibold leading-5 text-[#66735c]">{job.description}</p>
      <div className="mt-4 flex items-center justify-between text-sm">
        <span className="font-black">
          {job.budgetAmount} {job.budgetToken}
        </span>
        <span className="flex items-center gap-1 font-bold text-[#66735c]">
          <CalendarDays className="h-4 w-4" />
          {job.deadline ?? "Flexible"}
        </span>
      </div>
      <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[#c8ff45] px-3 py-1 text-xs font-black">
